...
#        self.maxp3 =  nn.MaxPool1d(3)
        #3x conv 64 2x 32 1x 16
        self.rnn = nn.LSTM(32, self.target_class, self.cell_count, dropout=self.dropout, batch_first=True)
        #3x conv 39 2x 127 1x 899
...
#        x = self.maxp3(x)
    ...
    def test_step(self, batch_data, batch_index):
        ...
        return {"test_loss": loss, "test_accuracy": acc, "f_score" : f_score, "predictions" : predictions, "targets" : targets}
    ...
    def test_epoch_end(self, test_step_outputs):
        ...
        combined_pred = torch.cat(
            [x["predictions"] for x in test_step_outputs], 0)
        combined_target = torch.cat(
            [x["targets"] for x in test_step_outputs], 0)
        print(classification_report(combined_target.cpu(), combined_pred.cpu(), target_names=target_names))
